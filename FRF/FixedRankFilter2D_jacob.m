function [Y_pred,var_pred,diff,binned_data] = FixedRankFilter2D_jacob(r,numBins,resolution, training1, training2, training1_var, training2_var, t1, t2, t3, t4, t5, t6)
    ...
    earthRadius = 6.371e6;
    t1 = load(t1); t2 = load(t2); t3 = load(t3); t4 = load(t4); t5 = load(t5); t6 = load(t6);
    lats = [t1.location(:,1); t2.location(:,1); t3.location(:,1); t4.location(:,1); t5.location(:,1); t6.location(:,1)];
    lons = [t1.location(:,2); t2.location(:,2); t3.location(:,2); t4.location(:,2); t5.location(:,2); t6.location(:,2)];
    minLat = min(lats); minLon = min(lons);
    xcoord = (lats - minLat) .* (pi / 180) .* cos((lons - minLon) / 2 .* pi / 180) .* earthRadius;
    ycoord = (lons - minLon) .* pi / 180 .* earthRadius;
    
    time = [t1.time; t2.time; t3.time; t4.time; t5.time; t6.time];
    time = time(:,10:end);
    time = datetime(time,'InputFormat','HH-mm-ss');
    dayof = 20*ones(length(time),1); monthof = 7*ones(length(time),1); yearof = 2018*ones(length(time),1);
    time.Year = yearof; time.Month = monthof; time.Day = dayof;
    
    aveconc = [str2num(t1.pol); str2num(t2.pol); str2num(t3.pol); str2num(t4.pol); str2num(t5.pol); str2num(t6.pol)];
    startTimesMat = [t1.time(1,:); t2.time(1,:); t3.time(1,:); t4.time(1,:); t5.time(1,:); t6.time(1,:); t6.time(end,:)];
    startTimesMat = startTimesMat(:,10:end);
    startTimesMat = datetime(startTimesMat,'InputFormat','HH-mm-ss');
    dayof = 20*ones(length(startTimesMat),1); monthof = 7*ones(length(startTimesMat),1); yearof = 2018*ones(length(startTimesMat),1);
    startTimesMat.Year = yearof; startTimesMat.Month = monthof; startTimesMat.Day = dayof;
    ...
    %values
    [~, binned_data, central_coord, combined_data] = binMaker2D(xcoord, ycoord, time, aveconc, time, numBins, startTimesMat);
