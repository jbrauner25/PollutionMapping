function [Y_pred,var_pred,diff,binned_data] = FixedRankFilter2Dbackup(nmea_file,mcpc_file,startTimesMat,r,numBins,resolution, training1, training2)
...
%         end
    ...
    subplot(2,4,5);
    surf(cc(:,:,2), cc(:,:,1), var(:,:,1));
    axis('square')
    xlabel('x position (m)');
    ylabel('y position (m)');
    zlabel('Variance ((#/cc)^{2})');
    title('Variance vs Position (Afternoon, t=1)');
    
    subplot(2,4,6);
    surf(cc(:,:,2), cc(:,:,1), var(:,:,2));
    axis('square')
    xlabel('x position (m)');
    ylabel('y position (m)');
    zlabel('Variance ((#/cc)^{2})');
    title('Variance vs Position (Afternoon, t=2)');
    
    subplot(2,4,7);
    surf(cc(:,:,2), cc(:,:,1), var(:,:,3));
    axis('square')
    xlabel('x position (m)');
    ylabel('y position (m)');
    zlabel('Variance ((#/cc)^{2})');
    title('Variance vs Position (Afternoon, t=3)');
    
    subplot(2,4,8);
    surf(cc(:,:,2), cc(:,:,1), var(:,:,4));
    axis('square')
    xlabel('x position (m)');
    ylabel('y position (m)');
    zlabel('Variance ((#/cc)^{2})');
    title('Variance vs Position (Afternoon, t=4)');
